...
let socket = io(`ws://${location.hostname}:3099`);
...
socket.on('playerDisconnect', data => {
  document.getElementById('waiting').innerHTML =
    '<span style="color: red;">对方掉线</span>';
...
socket.on('start', ({ myTurn, myNickname, hisNickname }) => {
  ...
  if (myTurn) {
    ...
    document.getElementById(
      'warning',
    ).innerHTML = `<span style="color: red;">请等待${gameVars.hisNickname}下子...</span>`;
    gameVars.myFirstStep = 0;
  } else {
    document.getElementById(
      'waiting',
    ).innerHTML = `游戏开始！请等待${gameVars.hisNickname}下子...`;
...
socket.on('switchPlayer', myTurn => {
  ...
  if (gameVars.myTurn) {
    ...
    document.getElementById(
      'waiting',
    ).innerHTML = `请等待${gameVars.hisNickname}下子...`;
  }
});
// 将自己的下子同步给对手
...
class Game extends React.Component {
  ...
  handleInit = () => {
    ...
    this.setState({
      history: [
        {
          squares: Array(this.props.rowNum)
            .fill(null)
            .map(o =>
              Array(this.props.colNum)
                .fill(null)
                .map(o2 => ({ player: null })),
            ),
        },
  ...
  handleClick = (x, y, switchPlayer) => {
    ...
    const _current = history[stepNumber].squares[y][x];
    console.log('_current:', _current);
    const isCurrentSquareFilled = !!history[stepNumber].squares[y][x].player;
    console.log('isCurrentSquareFilled:', isCurrentSquareFilled);
    if (!switchPlayer) {
      if (!gameVars.myTurn) {
        $('#warning')
    ...
    const finalHistory = nextHistory.concat([
      {
        squares: squares,
      },
    ]);
    if (this.judge(x, y, squares, this.state.xIsNext ? 'X' : 'O')) {
      this.setState({
        ...
        lastWinStep: finalHistory.length - 1,
        lastWinner: squares[y][x].player,
        history: finalHistory,
        stepNumber: finalHistory.length - 1,
